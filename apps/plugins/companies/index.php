...
$plugin_dir = "companies";
$content_group = "company";
...
switch ($path) {
    ...
    case "admin/{$plugin_dir}/delete/{$GLOBALS['url_last_param']}":
        ...
        break;
        case "admin/{$plugin_dir}/get-seller-list-ajax":
            $userObj = new Model('pk_user');
            // $employees = $userObj->index();
            $search_data = sanitize_remove_tags($_POST['seller_keyword']);
            $arr['id'] = $search_data;
            ...
            $employees = $userObj->search(assoc_arr:$arr,whr_arr:array('user_group'=>'seller'));
            if ($employees==false) {
                $employees = array();
            }
        ?>
        <?php foreach ($employees as $empk => $empv): ?>
            <li id="getEmpList<?php echo $empv['id']; ?>" class="py-1 pk-pointer">
                <input <?php if(isset($_POST['old_created_by']) && ($_POST['old_created_by']==$empv['id'])){echo "checked"; } ?> onclick="setInSearch(`<?php echo $empv['first_name']; ?>`)" class="update_page" type="radio" name="created_by" value="<?php echo $empv['id']; ?>"> 
